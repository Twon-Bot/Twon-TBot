...
#I want to fix several things in the morning, so keep this list for me:  

#1. I don't want **2 bar graph emojis** in the title, just **1**.  
#2. The **emojis still suck** in the embed and buttons. Buttons don't output emojis correctly, and for some reason, the emoji sizing (on phone) and black part (looking checkered) just don't want to work.  
#3. The **"multiple" poll** doesn't work due to this error message in command prompt:  
#   > `Failed to send interaction response: 'str' object has no attribute 'append'`  
#4. `!!export_votes` only **tracks the most recent poll.** I'd like **each poll to have a custom button** that I can click, which will output the CSV file for the results of **that specific poll.** That way, I can do multiple polls at once and track the info from each, while also making it easier to use the button rather than a typed-out function. This button would show a **gear/cog emoji.**  
#5. There's still that **extra space between the title and options.**  
#6. A **"mentions" option** that will include the ability to **@everyone** right above the poll.  
#7. A **+ (plus) emoji button** that allows me to **add an option** after creating the poll. Perhaps it could be an **edit button in general,** allowing me to remove or edit other options too.  
#8. In **very small writing at the bottom inside the embed,** I want it to show the **server nickname** of the user who made the poll, saying:  
#   > `"Made by {user nickname}."`
